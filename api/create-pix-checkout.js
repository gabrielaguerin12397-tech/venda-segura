...
module.exports = async function handler(request, response) {
  ...
  try {
    ...
    if (!isValidCpfCnpj(cpfCnpj)) {
      return response.status(400).json({ error: "Informe um CPF ou CNPJ valido para pagar com Pix." });
    }

    if (!phoneNumber) {
      return response.status(400).json({ error: "Informe um telefone valido com DDD para pagar com Pix." });
    }

    if (postalCode.length !== 8 || !address || !addressNumber || !province || !city || state.length !== 2) {
      return response.status(400).json({ error: "Informe endereco completo para pagar com Pix." });
    }
    ...
    const checkoutResponse = await fetch(`${ASAAS_API_URL}/checkouts`, {
      ...
      body: JSON.stringify({
        ...
        items: [
          {
            name: "Venda Segura - 30 dias",
            description: "Acesso por 30 dias ao Venda Segura via Pix",
            quantity: 1,
            value: PLAN_VALUE,
          },
        ],
    ...
    if (!checkoutResponse.ok) {
      return response.status(400).json({
        error: checkout.errors?.[0]?.description || "Nao foi possivel criar o Pix no Asaas.",
      });
    }
